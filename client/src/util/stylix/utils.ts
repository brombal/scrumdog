 ...
 */
export function postcssSerialize(obj: any, getStylixContext: StylixThemeContext | (() => StylixThemeContext)): string {
  if (obj === null || obj === undefined) return "";

  if (typeof obj === "string") return obj;

  if (Array.isArray(obj)) {
    const ctx: StylixThemeContext = typeof getStylixContext === "function" ? getStylixContext() : getStylixContext;
    if (!ctx)
  ...
  if (isPlainObject(obj)) {
    return Object.keys(obj).reduce((memo, key) => {
      ...
      if (
        ...
        (Array.isArray(obj[key]) && !obj[key].some((v) => typeof v === "string" || typeof v === "number"))
      ) {
        return memo + `\n${key}{${value}}`;
      }
      return memo + `\n${camelToHyphen(key)}:${value};`;
    }, "");
  }
 ...
 */
export async function postcssToRuleArray(css: string, plugins?: AcceptedPlugin[]): Promise<string[]> {
  const rules = [""];
  await postcss([postcssNested, postcssInlineMedia, postcssDefaultUnit, postcssDuplicates, ...(plugins || [])]).process(
    css,
    {
...
export async function applyContextRules(ctx: StylixSheetContext) {
  ...
  if (!newRules.every((v, i) => ctx.currentRules[i] === newRules[i])) {
    ...
    if (container instanceof HTMLStyleElement && container.tagName?.toLowerCase() === "style") {
      ...
      ctx.currentRules.filter((r) => !!r.trim()).forEach((rule, i) => container.insertRule(rule, i));
    }
  }
}
