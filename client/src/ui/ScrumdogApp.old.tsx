...
import { EventEmitter, EventProvider } from "../util/useEventEffect";
...
export default function ScrumdogApp({ ...other }: StylixProps) {
  ...
  const [eventContext] = useState(new EventEmitter());
  ...
  useEffect(() => {
    (async () => {
      ...
      let urlRoomCode = location.replace(/^\//, "");
  ...
  store.useEffect(
    (state) => {
      if (!state.state) window.history.replaceState(null, null, "/" + (state.room?.code || ""));
    },
